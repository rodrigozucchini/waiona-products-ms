...
export class CalculationService {
  ...
  async calculateProduct(dto: CalculateProductDto): Promise<PriceBreakdownDto> {
    ...
    if (!pricing) {
      throw new RpcException({
        status: 404,
        message: 'Pricing de producto no encontrado',
      });
    }
  ...
  async calculateCombo(dto: CalculateComboDto): Promise<PriceBreakdownDto> {
    ...
    if (!pricing) {
      throw new RpcException({
        status: 404,
        message: 'Pricing de combo no encontrado',
      });
    }
  ...
  private applyValue(
    ...
    if (isNaN(value)) {
      throw new RpcException({
        status: 500,
        message: `Valor de porcentaje inválido en cálculo de precio: ${value}`,
      });
    }
  ...
  private async fetchTaxDataForCombo(comboId: number): Promise<{
    ...
    const itemsWithRef = items.map((item) => {
      if (!pricingMap.has(item.productId)) {
        throw new RpcException({
          status: 404,
          message: `El producto ${item.productId} del combo no tiene pricing configurado`,
        });
      }
      return {
