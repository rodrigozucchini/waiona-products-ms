...
export class CategoryService {
  ...
  async create(dto: CreateCategoryDto): Promise<CategoryResponseDto> {
    ...
    try {
      ...
      if (err instanceof QueryFailedError) {
        throw new RpcException({
          status: 409,
          message: `Ya existe una categoría con el nombre "${dto.name}"`,
        });
      }
      throw err;
    }
  ...
  async update(
    ...
    try {
      ...
      if (err instanceof QueryFailedError) {
        throw new RpcException({
          status: 409,
          message: `Ya existe una categoría con el nombre "${changes.name}"`,
        });
      }
      throw err;
    }
  ...
  async delete(id: number): Promise<void> {
    ...
    if (blocking.length > 0) {
      throw new RpcException({
        status: 409,
        message: `No se puede eliminar la categoría: tiene ${blocking.join(', ')} asignado(s)`,
      });
    }
