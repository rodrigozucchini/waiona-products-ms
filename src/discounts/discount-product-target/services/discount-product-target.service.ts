...
export class DiscountProductTargetService {
  ...
  async remove(discountId: number, productId: number): Promise<void> {
    ...
    if (!entity) {
      throw new RpcException({
        status: 404,
        message: `El producto ${productId} no está asignado al descuento ${discountId}`,
      });
    }
  ...
  private async validateUniqueTarget(
    ...
    if (existing) {
      throw new RpcException({
        status: 409,
        message: `El producto ${productId} ya es un target del descuento ${discountId}`,
      });
    }
  }
  ...
  private async validateProductHasNoActiveDiscount(
    ...
    if (existing) {
      throw new RpcException({
        status: 409,
        message: `El producto ${productId} ya tiene un descuento asignado`,
      });
    }
  }
}
